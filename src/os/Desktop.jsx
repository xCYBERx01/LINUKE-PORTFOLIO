...
export function BootScreen() {
  ...
  const bootLines = [
    "Loading Mintex Linux kernel 6.6.1...",
    "Enumerating USB devices... ok",
    "Mounting /home/ahmed... ok",
    "Starting Mintex Shell...",
  ];
  ...
  const line = Math.min(Math.floor(progress / 25), bootLines.length - 1);

  return (
    <div className="os-boot">
      <div className="os-boot-logo">
        <Monitor size={44} />
        <span>Mintex Linux</span>
      </div>
...
/* ---------------- Login ---------------- */
export function LoginScreen({ onLogin, onPower }) {
  ...
  function submit(e) {
    ...
    if (v === "linux" || v === "mintex" || v === "password" || v === "ahmed") {
      onLogin();
    } else {
      setError(true);
      setPass("");
  ...
  return (
    <div className="os-login">
      <form className="os-login-card" onSubmit={submit}>
        ...
        <small>Hint: linux · mintex · password · ahmed</small>
      </form>
