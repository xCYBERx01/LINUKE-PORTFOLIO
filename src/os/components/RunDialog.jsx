...
export function RunDialog({ open, onClose, onOpen }) {
  ...
  useEffect(() => {
    ...
    if (!q) { setSuggestions(Object.entries(RUN_COMMANDS).slice(0, 6).map(([key, cmd]) => ({ key, ...cmd }))); return; }
    const fromRegistry = desktopApps()
      .filter((a) => a.label.toLowerCase().includes(q) || a.appId.toLowerCase().includes(q))
      .slice(0, 6)
      .map((a) => ({ key: a.appId, label: a.label, icon: a.icon, appId: a.appId }));
    const fromCommands = Object.entries(RUN_COMMANDS)
      .filter(([key, cmd]) => key.includes(q) || cmd.label.toLowerCase().includes(q))
      .slice(0, 4)
      .map(([key, cmd]) => ({ key, ...cmd }));
    // merge dedup
    const merged = [...fromRegistry];
    for (const c of fromCommands) if (!merged.find((m) => m.appId === c.appId)) merged.push(c);
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="os-run-form">
          ...
          <div className="os-run-hint">
            <kbd>⌘</kbd>+<kbd>R</kbd> to close · <kbd>Esc</kbd> to cancel · <kbd>?</kbd> for help · try “paint” or “croc” or a URL
          </div>
        </form>
      </motion.div>
    </Draggable>
