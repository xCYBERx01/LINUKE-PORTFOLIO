  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!mounted) return;
        const cur = Number(spot.data.data.amount);
        setPrice(spot.data.data.amount);
        // build a tiny sparkline from single point + jitter (avoid failing candles API)
        const seed = Array.from({ length: 20 }, (_, i) => ({
          time: `${i}`,
          price: cur * (0.98 + Math.random() * 0.04),
        }));
        setHistory(seed);
        try {
          const candles = await axios.get("https://api.exchange.coinbase.com/products/BTC-USD/candles?granularity=300");
          if (mounted && Array.isArray(candles.data) && candles.data.length) {
            const data = candles.data.slice(0, 30).reverse().map((c) => ({
...
const CRICKET_KEY = import.meta.env.VITE_CRICKET_KEY || "";
...
function FootballWidget() {
  ...
  useEffect(() => {
    if (!FOOTBALL_KEY) { setErr("Add VITE_FOOTBALL_KEY to .env for live"); return; }
    let mounted = true;
    async function load() {
      try {
...
function CricketWidget() {
  ...
  useEffect(() => {
    if (!CRICKET_KEY) { setErr("Add VITE_CRICKET_KEY to .env for live"); return; }
    let mounted = true;
    async function load() {
      try {
